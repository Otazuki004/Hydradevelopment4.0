...
from Hydra1 import ND
from Hydra1.database import find, insert
from Hydra1.helper import convert, progress_for_pyrogram
...
async def rename(_, message):
    ...
    try:
        await message.reply_text(
            text=text, reply_markup=button, reply_to_message_id=message.id
        )
    except Expection:
...
async def cb_rename(_, query):
    ...
    await query.message.reply_text(
        '🗒️ Enter File name also enter with type (".MKV",".MP4",".MP3"):-',
...
async def refund(_, message):
    ...
    if (reply.reply_markup) and isinstance(reply.reply_markup, ForceReply):
        ...
        button = [
            [InlineKeyboardButton("📁 𝙳𝙾𝙲𝚄𝙼𝙴𝙽𝚃𝚂", callback_data="upload:document")]
        ]
        if str(media) in ["MessageMediaType.VIDEO", "MessageMediaType.DOCUMENT"]:
            button.append(
                [InlineKeyboardButton("🎥 𝚅𝙸𝙳𝙴𝙾", callback_data="upload:video")]
            )
        elif str(media) == "MessageMediaType.AUDIO":
            button.append(
                [InlineKeyboardButton("🎵 𝙰𝚄𝙳𝙸𝙾", callback_data="upload:audio")]
            )
        await message.reply_text(
